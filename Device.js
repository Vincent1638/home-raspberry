...
module.exports = class Device {
  ...
  createDevice(info) {
    console.log(info)
    const tuya = new TuyAPI({ id: info.id, key: info.key, ip: info.ip })
    tuya.start = async function () {
      try {
        ...
        console.error('Unable to connect:', info.id, info.name[0])
        setTimeout(() => this.start(), 20000)
      }
    }
    tuya.on('disconnected', () => {
      console.error('Disconnected:', info.id, info.name[0])
      setTimeout(() => tuya.start(), 20000)
    })
    tuya.on('error', () => {
      console.error('Error:', info.id)
    })
    tuya.start()
    ...
    switch (info.type) {
      ...
      case 'dimmer':
        const dimmer = {
          ...
          command: function (command) {
            ...
            this.tuya.set({ multiple: true, data: data }).catch(() => console.log('Error'))
          },
        }
      ...
      case 'outlet':
        const outlet = {
          ...
          command: function (command) {
            if (command.hasOwnProperty('state')) {
              this.tuya.set({ dps: '1', set: command.state }).catch(() => console.log('Error'))
            }
          },
        }
